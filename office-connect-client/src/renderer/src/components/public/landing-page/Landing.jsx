...
const Landing = ({ selectedFriendProfileId, userFullDetails }) => {
 ...
 useEffect(() => {
  ...
  var_conn.on("ReceiveMessage", (msg) => setMessages(prev => [...prev, msg]));

  setConnection(var_conn);
  return () => var_conn.stop();
  ...
  return (
    <div className="chat">
      <div className="messages" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
        {messages.map((m, i) => (
          <div key={i}>
            <b>{m.sender_id === currentUserId ? 'Me' : 'Them'}:</b> {m.message_text}
          </div>
        ))}
      </div>
      ...
      <button
        ...
        className="bg-slate-100 border border-slate-50 cursor-pointer"
      >
        Send
      </button>
    </div>
